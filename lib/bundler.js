...
{
    "1": <requested schema with internal refs>,
    "2": <additional schema with internal refs>,
    "3": <additional schema with internal refs>,
    ...
}
...
The JSON-Schema of bundle would be:
...
function bundle(originPointer, originSchema, pointerResolver) {
    ...
    var addToBundle = function (pointer, schema) {
        ...
        bundle[id] = JSON.parse(JSON.stringify(schema)); // deep copy
        queue.push(id);
        return id;
    };
    ...
    var recusiveBundle = function(localPointerId, schemaPart) {
        if (schemaPart['$ref']) {
            ...
            if (pointer.isInternal) {
                // repoint the ref
                var newRef = '#/' + localPointerId;
                if (pointer.fragment) {
                    newRef += pointer.fragment;
                }
                schemaPart['$ref'] = newRef;
            }
            else {
...
function derefSchemaBundle(bundleDocument) {
    ...
    recusiveDeref(null, null, bundleDocument['1']);
    return bundleDocument['1'];
}
